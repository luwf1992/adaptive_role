...
class HriEnv_v1(gym.Env):

    def __init__(self):
        self.dt = 0.05
        self.g = 0.
        self.task_num = 2
        self.x_g1 = np.array([0.2, 0.7, 0.06561195563238487])
        self.x_g2 = np.array([-0.1, 0.3, 0.06561195563238487])
        self.x_g = np.concatenate((self.x_g1, self.x_g2), axis=0).reshape(self.task_num, 3)
        self.m_r = 10.
        self.d_r = 20.
    ...
    def step(self, b_list):
        ...
        self.ee_pos, ee_orn, _, _, _, _, self.ee_vel, _ = p.getLinkState(
            self.panda, self.pandaEndEffectorIndex, computeLinkVelocity=1, computeForwardKinematics=1)
        ee_pos_array = np.array(self.ee_pos).reshape(3, 1)
        self.ee_vel_array = np.array(self.ee_vel).reshape(3, 1)
        cov_b = 0.
...
if __name__ == "__main__":
    ...
    print('x_d')
    print(env.x_d)
    b_list = [1.-env.task_index, env.task_index]
    print('b_list')
    print(b_list)
    p_x_list = []
    p_y_list = []
